...
export const RegalAnvika: React.FC<Props> = ({ profile }) => {
  ...
  return (
    <div className="w-full h-full bg-[#3d1024] flex text-[#fdf2f8] font-cormorant relative min-h-full overflow-hidden">
         {/* Decorative Border */}
         ...
         <div className="w-[38%] border-r border-[#d4af37]/30 p-5 flex flex-col bg-[#2a0a18] relative min-h-full shrink-0">
            ...
            <div className="relative z-10 flex flex-col w-full h-full gap-3 min-h-0">
                ...
                <div className="w-full space-y-3 text-left text-xs leading-relaxed flex-1 min-h-0 overflow-y-auto">
                    <div>
         ...
         {/* Right Side - 60% */}
         <div className="w-[62%] p-8 flex flex-col relative gap-6 min-h-full overflow-hidden">
            <div className="shrink-0 border-b border-[#d4af37]/20 pb-4">
              <h1 className="text-5xl text-[#d4af37] font-great-vibes leading-tight mb-2">{personal.fullName}</h1>
              <p className="text-xs uppercase tracking-[0.28em] text-[#fbcfe8] opacity-80">{education.occupation}</p>
            </div>

            <div className="space-y-8 flex-1 min-h-0 overflow-y-auto pr-2">
                <section>
                    ...
                    <div className="grid grid-cols-2 gap-y-3 gap-x-4 text-sm">
                         ...
                         {education.aboutMe && <div className="flex flex-col col-span-2"><span className="text-[#d4af37] text-xs uppercase opacity-70 mb-1">{t('field.aboutMe')}</span> <span className="text-sm italic normal-case opacity-90 leading-relaxed whitespace-pre-line">{education.aboutMe}</span></div>}
                    </div>
                </section>
                ...
                <section className="pb-4">
                    ...
                    <div className="space-y-3 text-sm border-l-2 border-[#d4af37]/30 pl-4">
                         {family.nativePlace && (
                            <div>
                                <span className="text-[#d4af37] text-sm uppercase block tracking-wider opacity-70 mb-1">{t('field.nativePlace')}</span>
                                <span className="text-sm">{family.nativePlace}</span>
                            </div>
                         )}
                         {family.fatherName && (
                            <div>
                                <span className="text-[#d4af37] text-sm uppercase block tracking-wider opacity-70 mb-1">{t('field.fatherName')}</span>
                                <span className="text-sm">{family.fatherName}</span>
                                {family.fatherOccupation && <span className="text-sm opacity-60 block mt-0.5">{family.fatherOccupation}</span>}
                            </div>
                         )}
                         {family.motherName && (
                            <div>
                                <span className="text-[#d4af37] text-sm uppercase block tracking-wider opacity-70 mb-1">{t('field.motherName')}</span>
                                <span className="text-sm">{family.motherName}</span>
                                {family.motherOccupation && <span className="text-sm opacity-60 block mt-0.5">{family.motherOccupation}</span>}
                            </div>
                         )}
                         {(family.familyType || family.familyValues) && (
                            <div className="grid grid-cols-2 gap-4">
                                {family.familyType && (
                                    <div>
                                        <span className="text-[#d4af37] text-sm uppercase block tracking-wider opacity-70 mb-1">{t('field.familyType')}</span>
                                        <span className="text-sm">{family.familyType}</span>
                                    </div>
                                )}
                                {family.familyValues && (
                                    <div>
                                        <span className="text-[#d4af37] text-sm uppercase block tracking-wider opacity-70 mb-1">{t('field.values')}</span>
                                        <span className="text-sm">{family.familyValues}</span>
                                    </div>
                                )}
                            </div>
                         )}
                         {family.siblings && (
                            <div>
                                <span className="text-[#d4af37] text-sm uppercase block tracking-wider opacity-70 mb-1">{t('field.siblings')}</span>
