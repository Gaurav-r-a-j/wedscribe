...
import { TEMPLATE_DISPLAY_LIST } from '@/templateConfigs';
...
export const EditorPage: React.FC = () => {
  ...
  const TemplateCard = ({ type, name, colorClass, badge }: { type: TemplateType, name: string, colorClass: string, badge?: string }) => (
    <button
      ...
      className="group flex flex-col items-center gap-3 text-center transition-all duration-300 w-full"
    >
      <div className={`w-full aspect-[1/1.4] rounded-xl overflow-hidden relative transition-all shadow-md group-hover:shadow-xl ${activeTemplate === type ? 'ring-4 ring-primary shadow-primary/20' : 'ring-1 ring-slate-200 group-hover:ring-primary/30'}`}>
        <div className={`w-full h-full ${colorClass} p-4 flex flex-col`}>
          <div className="w-full h-1/4 bg-current opacity-10 rounded-lg mb-3" />
          <div className="flex gap-2 grow">
            <div className="w-full h-full bg-current opacity-5 rounded-lg border border-current/10" />
          </div>
        </div>
        ...
        {badge && (
          <div className="absolute bottom-3 left-3 bg-white/95 text-[10px] font-bold text-slate-700 px-2 py-1 rounded-full shadow-md border border-slate-200">
            {badge}
          </div>
        )}
      </div>
  ...
  const TemplatesGrid = () => (
    <div className="grid grid-cols-2 gap-x-4 gap-y-8 pb-20 md:pb-0 max-h-[calc(100vh-220px)] overflow-y-auto pr-1">
      ...
      {/* Config-based templates - auto-rendered */}
      {TEMPLATE_DISPLAY_LIST.map((template) => (
        <React.Fragment key={template.id}>
          <TemplateCard
            type={template.id as TemplateType}
            name={template.name}
      ...
      <TemplateCard type={TemplateType.BACKGROUND_SHOWCASE} name="BG Showcase" colorClass="bg-green-900 text-emerald-100" badge="Switcher" />
    </div>
  );
